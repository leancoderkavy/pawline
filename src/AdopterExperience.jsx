...
function Discovery({ pets, profile, saved, onSave, onOpen, onOpenMap }) {
...
export default function AdopterExperience({ pets, saved, onSave, clerkConfigured, isSignedIn = false, initialView = "discover", authAction = null, onOpenMap, onOpenMessages, getToken }) {
  ...
  const currentPage = selectedPet
    ...
    : view === "discover" ? <Discovery pets={pets} profile={profile} saved={saved} onSave={onSave} onOpen={openPet} onOpenMap={onOpenMap} />
  ...
  return <div className="adopter-experience">
    <header className="journey-header">
      ...
      <nav aria-label="Adopter navigation"><NavButton active={view === "home" && !selectedPet} icon={House} onClick={() => navigate("home")}>Home</NavButton><NavButton active={view === "discover" || Boolean(selectedPet)} icon={Search} onClick={() => navigate("discover")}>Discover</NavButton><NavButton active={view === "applications"} icon={ClipboardList} onClick={() => navigate("applications")}>Applications</NavButton><NavButton active={view === "messages"} icon={MessageCircle} onClick={() => navigate("messages")}>Messages</NavButton><NavButton active={view === "profile"} icon={UserRound} onClick={() => navigate("profile")}>Profile</NavButton></nav>
      <div className="journey-header-actions">{authAction}<button type="button" className="journey-map-link" aria-label="Open adoption map" onClick={onOpenMap}><Map /> <span>Map</span></button></div>
    </header>
    <nav className="journey-bottom-nav" aria-label="Adopter navigation">{[["home", House, "Home"], ["discover", Search, "Discover"], ["applications", ClipboardList, "Applications"], ["messages", MessageCircle, "Messages"], ["profile", UserRound, "Profile"]].map(([key, Icon, label]) => <NavButton key={key} active={view === key && !selectedPet} icon={Icon} onClick={() => navigate(key)}>{label}</NavButton>)}</nav>
