...
function Discovery({ pets, searchLocation, searchDistance, profile, saved, onSave, onOpen, onOpenMap, onRecenter, feed }) {
  ...
  const suggestedCenter = feed?.suggestedCenter;
  const emptyHeading = isLoading ? "Current listings are loading." : unavailable ? "Current listings are temporarily unavailable." : suggestedCenter ? `No pets found within ${searchDistance} miles of ${searchLocation}.` : "No current listings match this view.";
...
function Profile({ profile, onChange, onSave, isSignedIn, onOpenAuth, saving }) {
  const [collaborator, setCollaborator] = useState("");
  const update = (key, value) => onChange({ ...profile, [key]: value });
  const addCollaborator = () => {
    const name = collaborator.trim();
    if (!name || profile.collaborators.length >= 4 || profile.collaborators.some(item => item.toLowerCase() === name.toLowerCase())) return;
    onChange({ ...profile, collaborators: [...profile.collaborators, name].slice(0, 4) });
    setCollaborator("");
  };
  return <section className="journey-content profile-page" aria-labelledby="profile-title"><header className="journey-page-heading"><div><p className="eyebrow">Profile</p><h1 id="profile-title">Tell us about the life you can share.</h1><p>Use this to clarify matches. Pawline does not turn it into an applicant score.</p></div></header><div className="profile-grid">{PROFILE_FIELDS.map(field => <label key={field.key}>{field.label}<select value={profile[field.key]} onChange={event => update(field.key, event.target.value)}>{field.options.map(option => <option key={option} value={option}>{option || "Choose one"}</option>)}</select></label>)}<label>Preferred distance<select value={profile.distance} onChange={event => update("distance", event.target.value)}><option value="25">Within 25 miles</option><option value="50">Within 50 miles</option><option value="100">Within 100 miles</option></select></label><label>Accessibility or visit needs (optional)<input value={profile.accessibility} maxLength="160" onChange={event => update("accessibility", event.target.value)} placeholder="e.g. step-free visit access" /></label></div><section className="household-panel"><div><UsersRound /><div><h2>Household context</h2><p>One verified adult owns an application. Pawline records these names as your private notes; collaborator invitations are not enabled in this release.</p></div></div><label>Household name (optional)<input value={profile.householdName} maxLength="80" onChange={event => update("householdName", event.target.value)} placeholder="The Martinez household" /></label><div className="collaborator-row"><input aria-label="Household member first name" value={collaborator} onChange={event => setCollaborator(event.target.value)} onKeyDown={event => { if (event.key === "Enter") { event.preventDefault(); addCollaborator(); } }} maxLength="80" placeholder="Add a household member’s first name" /><button type="button" onClick={addCollaborator} disabled={!collaborator.trim() || profile.collaborators.length >= 4 || profile.collaborators.some(name => name.toLowerCase() === collaborator.trim().toLowerCase())}>Add</button></div>{profile.collaborators.length ? <ul>{profile.collaborators.map(name => <li key={name}>{name}<button type="button" onClick={() => onChange({ ...profile, collaborators: profile.collaborators.filter(item => item !== name) })} aria-label={`Remove ${name}`}>Remove</button></li>)}</ul> : null}</section><div className="profile-save"><button type="button" disabled={saving} onClick={() => !isSignedIn && onOpenAuth ? onOpenAuth("signin") : onSave()}><Check /> {saving ? "Saving preferences…" : isSignedIn ? "Save preferences" : "Sign in to save private details"}</button><p>{isSignedIn ? "Save private preferences and household context to your Pawline account." : "You can explore without an account. Only non-sensitive matching preferences are saved in this browser."}</p></div></section>;
}
...
function ApplicationMessages({ applications, isSignedIn, getToken, onDiscover, onOpenAuth }) {
  ...
  if (!isSignedIn) return <section className="journey-content"><header className="journey-page-heading"><div><p className="eyebrow">Messages</p><h1>Keep adoption conversations private.</h1><p>Sign in before Pawline can show or send application-linked messages.</p></div></header><div className="journey-empty"><MessageCircle /><h2>Private messages need an account.</h2><p>Guest browsing stays open, but no anonymous application conversation is created.</p>{typeof onOpenAuth === "function" ? <div className="journey-guest-auth-actions"><button type="button" onClick={() => onOpenAuth("signup")}>Create account</button><button type="button" className="outline-action" onClick={() => onOpenAuth("signin")}>Sign in</button></div> : null}</div></section>;
...
export default function AdopterExperience({ pets, nearbyPets = pets, searchLocation, searchDistance, saved, onSave, isSignedIn = false, view = "home", active = true, onNavigate, applicationPet, onApplicationHandled, onOpenMap, onOpenMessages, onOpenAuth, onRecenter, clerkConfigured, getToken, feed }) {
  ...
  const saveProfile = async () => {
    ...
    if (privateState.status === "saving") return;
    setPrivateState({ status: "saving", message: "Saving your private profile…" });
    try {
  ...
  const currentPage = selectedPet
    ...
    : view === "discover" ? <Discovery pets={nearbyPets} searchLocation={searchLocation} searchDistance={searchDistance} feed={feed} profile={profile} saved={saved} onSave={onSave} onOpen={openPet} onOpenMap={onOpenMap} onRecenter={onRecenter} />
      : view === "applications" ? <Applications applications={applications} selectedApplication={applications.find(item => item.id === selectedApplicationId)} checkins={checkins} isSignedIn={isSignedIn} getToken={getToken} onSelect={setSelectedApplicationId} onUpdate={updateApplication} onSaveDraft={saveDraft} onSubmitApplication={submitApplication} onConfirmOutcome={confirmAdoptionOutcome} onConfirmCheckin={confirmPlacementCheckin} onDiscover={() => navigate("discover")} />
        : view === "messages" ? <ApplicationMessages applications={applications} isSignedIn={isSignedIn} getToken={getToken} onDiscover={() => navigate("discover")} onOpenAuth={onOpenAuth} />
          : view === "profile" ? <><Profile profile={profile} onChange={updateProfile} onSave={saveProfile} isSignedIn={isSignedIn} onOpenAuth={onOpenAuth} saving={privateState.status === "saving"} />{privateState.message ? <p className={privateState.status === "error" ? "journey-private-error" : "journey-private-state"} role={privateState.status === "error" ? "alert" : "status"}>{privateState.message}</p> : null}</>
            : <Home profile={profile} applications={applications} pets={nearbyPets} saved={saved} onSave={onSave} onOpen={openPet} onNavigate={navigate} isSignedIn={isSignedIn} />;
