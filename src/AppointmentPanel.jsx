...
export default function AppointmentPanel({ request, conversation, onClose }) {
  ...
  const confirmationEmail = row => emailOptIn[`${row.id}:${row.revision}`] ?? row.emailReminder;
      ...
      {call ? <Suspense fallback={<p role="status">Opening video hello…</p>}><DailyCall request={request} conversation={conversation} appointment={call} onLeave={() => { setCall(null); refresh().catch(() => {}); }} onFinish={() => finish(call)} /></Suspense> : <>
        {!data ? <p role="status">Loading appointments…</p> : paused ? <p role="status">Reopen or unblock this conversation to arrange a meeting.</p> : form ? <AppointmentForm key={form.current?.id || 'new'} current={form.current} initialKind={form.initialKind} videoEnabled={data.videoEnabled} busy={busy} onSubmit={propose} onCancel={() => setForm(null)} /> : <>
          ...
          {data.appointments.map(row => <article key={row.id} className="appointment-card" aria-label={`${row.kind === 'video' ? 'Video hello' : 'Final visit'} appointment`}>
            ...
            {row.canAccept ? <><label className="appointment-checkbox"><input type="checkbox" checked={confirmationEmail(row)} onChange={event => setEmailOptIn(current => ({ ...current, [`${row.id}:${row.revision}`]: event.target.checked }))} />Email me updates and a reminder</label><button type="button" className="button" disabled={busy} onClick={() => action('accept', row, { emailReminder: confirmationEmail(row) })}>Confirm this time</button></> : row.state === 'proposed' ? <p className="appointment-hint">The other person needs to confirm this time.</p> : null}
            ...
            {row.canEdit ? <>{!row.canAccept ? <label className="appointment-checkbox"><input type="checkbox" checked={row.emailReminder} disabled={busy} onChange={event => action('reminders', row, { emailReminder: event.target.checked })} />Email me appointment updates and reminders</label> : null}<div className="appointment-actions">{['proposed', 'confirmed'].includes(row.state) ? <><button type="button" disabled={busy} onClick={() => setForm({ current: row })}>Suggest another time</button><button type="button" disabled={busy} onClick={() => action('cancel', row)}>Cancel appointment</button></> : null}{['confirmed', 'elapsed'].includes(row.state) && (new Date(row.startsAt) <= new Date() || row.videoOpened) ? <button type="button" disabled={busy} onClick={() => finish(row)}>Mark appointment finished</button> : null}</div></> : null}
