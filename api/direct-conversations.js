  ...
  return directEndpoint(["GET", "POST", "PATCH"], async ({ request, response, database, user, notify, environment }) => {
    ...
    if (request.method === "PATCH") {
      ...
      if (action === "block" || action === "resolve") {
        ...
        const rooms = await database`SELECT * FROM adoption_appointments WHERE conversation_id = ${conversation.id} AND room_name IS NOT NULL AND NOT room_closed`;
        for (const room of rooms) {
          const revoked = await revokeAppointmentRoom(database, room);
          try {
            await closeAppointmentRoom(database, dependencies?.provider || dailyProvider(environment), revoked);
