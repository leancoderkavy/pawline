...
- Conversations: participants can propose, confirm, and cancel upcoming meet-and-greet times. Existing chat and WebRTC remain in place. Meeting updates appear within the conversation; they do not send calendar invitations.
...
Tests cover importer snapshot failures, source identity, repeated and rolled-back migrations, catalog pagination, geographic filtering, account isolation, private tips, shelter administrator authorization and pending imports, and conversation membership and meeting confirmation. Browser coverage exercises existing chat/video with two local fixture users and fake media, mobile intake, and meeting proposal/confirmation. Test fixtures use a disposable database and are not part of production authentication.

Production health reported video calling unconfigured before this release. Reliable calls across restrictive networks require production TURN credentials and a two-account call across separate networks. Local WebRTC success is not evidence of that production behavior. Provider credentials, incoming outreach replies, and external data agreements remain operational dependencies.
