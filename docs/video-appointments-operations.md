...
- `api/appointments.js`: authenticated scheduling, revision checks, overlap prevention across either participant role, reminder preferences, room access, and adoption intentions. One upcoming appointment per conversation. Times are UTC instants with the originating IANA zone. Appointments last 15, 20, or 30 minutes, up to 60 days ahead. Only one join request creates the room; simultaneous callers receive a retry message until it is ready. Later joins never recreate rooms. Access is checked again after token creation, before returning credentials.
- `api/_appointment-rooms.js`: shared revocation and closure. Blocking, resolving, or finishing revokes application access before provider cleanup. A room still being created has a 60-second grace period; its creator closes it as soon as creation finishes if access was revoked. Maintenance handles an abandoned creator or provider failure.
...
- `src/AppointmentPanel.jsx`: booking, confirmation, local time display, calendar download, explicit email opt-in, embedded calls, and post-appointment intentions. Confirmation preserves the user's existing email preference across a counter-proposal and presents one email control. Calendar files contain no call credentials or private notes; imported calendar copies require a fresh download after changes.
- `api/cron/appointment-reminders.js`: five-minute maintenance, provider room closure, and leased/idempotent Resend delivery. Recipients must still have verified account email, conversation access, and a current enabled preference, checked again after account lookup. Superseded revisions are discarded. Re-enabling emails revives unsent discarded messages without resending delivered messages; opt-in within 30 minutes of the start queues one immediate reminder. The worker stops starting room work after 15 seconds and message work after 45 seconds, claiming one notification at a time so unattempted work keeps its retry allowance. A room cleanup failure does not stop unrelated reminders. `failed`, `closeFailed`, and `exhausted` counts and aggregate warnings expose problems without private error details.
...
Set `PAWLINE_DAILY_ENABLED=false` to stop new room access. With the API key retained, scheduled maintenance closes outstanding rooms; room expiry independently bounds their lifetime. Blocking/resolving a conversation cancels opened video appointments before attempting provider closure. Finishing remains saved if Daily is unavailable, with maintenance retry for media closure. In-flight creation is allowed to settle before closure to prevent a late orphaned room. Client authorization checks run every ten seconds; a malicious client can retain media until provider closure/room expiry if the provider is unavailable. Existing data is preserved and the additive migration need not be rolled back.

Inspect authenticated cron results for nonzero `failed`/`closeFailed`/`exhausted`, provider errors/usage in Daily, and Resend delivery status. Investigate before retrying exhausted notifications; do not reset attempts blindly or email opted-out users. An opened video appointment cannot be rescheduled: finish it, then create a new appointment, so an old room cannot be reused for another time.
